...
var BASE_CLASS, CLASSES, Card, STATES, VARIANTS, breakpoints, detectIE;
...
STATES = {
  BASE: {
    IS_VISIBLE: CLASSES.BASE + "--is-visible"
  },
  REVEALER: {
    IS_VISIBLE: CLASSES.TEXT_REVEALER + "--is-visible"
  }
};

VARIANTS = {
  BASE: {
    WITH_IMAGE: CLASSES.BASE + "--with-image"
  }
};

Card = function(node) {
  var $details, $image, $text_paragraph, $text_revealer, image, pre_flip_reset, size_card_to_image;
  ...
  size_card_to_image = function() {
    var card_header_height;
    ...
    return $details.innerHeight(image.offsetHeight - card_header_height);
  };
  pre_flip_reset = function() {
    var $text_container, text_container_height, text_paragraph_height;
    ...
    if (!$text_container.hasClass("text-container--is-open")) {
      return $text_revealer.toggleClass(STATES.REVEALER.IS_VISIBLE, text_paragraph_height > text_container_height);
    }
  };
  $(window).on("load", function() {
    if ($image[0].complete) {
      $image.load();
      return $(node).addClass(STATES.BASE.IS_VISIBLE);
    }
  });
  $(window).on("load resize", function() {
    if (detectIE.is_internet_explorer() && detectIE.get_internet_explorer_version() < 10) {
      if ($(window).width() >= 720) {
        return size_card_to_image();
      } else {
        return pre_flip_reset();
      }
...
$(function() {
  return $("." + VARIANTS.BASE.WITH_IMAGE).each(function(index, card) {
    return Card(card);
  });
});


},{"../utilities/breakpoints.coffee":11,"../utilities/detect_ie_version.coffee":12}],2:[function(require,module,exports){
require("./card.coffee");
...
},{"./card.coffee":1,"./image_gallery.coffee":3,"./image_grid.coffee":4,"./modals.coffee":5,"./social_popover.coffee":6,"./text_container.coffee":7}],3:[function(require,module,exports){
var CLASSES, Photoswipe, PhotoswipeUI, STATES, add_natural_size, apply_active_state, image_details;
...
STATES = {
  DOT: {
    IS_ACTIVE: CLASSES.DOT + "--is-active"
  },
  CARD: {
    ACTIVE_GALLERY: CLASSES.CARD + "--has-active-image-gallery"
  },
  CLOSE_BUTTON: {
    IS_ACTIVE: CLASSES.CLOSE_BUTTON + "--is-active"
  }
};

add_natural_size = function(detail) {
  var natural_image;
  natural_image = new Image;
  natural_image.onload = function() {
    detail.h = natural_image.height;
    return detail.w = natural_image.width;
  };
...
image_details = function() {
  var details;
  details = [];
  $("#extra-images img").each(function(i, img) {
    var detail;
    detail = {
      src: img.src,
      msrc: img.src,
...
apply_active_state = function(current_index) {
  $("." + CLASSES.NAV_LINK).each(function(i) {
    var add_active_class, index, link;
    link = $(this);
    ...
    return link.find("." + CLASSES.DOT).toggleClass(STATES.DOT.IS_ACTIVE, add_active_class);
  });
};

$(function() {
  var $close_button, $document, $photoswipe, new_index, thumbnail_details;
  $document = $(document);
  $photoswipe = $(".pswp");
  $close_button = $("." + CLASSES.CLOSE_BUTTON);
  thumbnail_details = image_details();
  new_index = 0;
  return $document.on("click", ".image--featured", function() {
    var $card, $card_image_container, options, photoswipe;
    $card = $("." + CLASSES.CARD);
    ...
    $document.on("click", "." + CLASSES.NAV_LINK, function() {
      var nav_target;
      nav_target = parseInt($(this).attr("data-nav-index"), 10);
      return photoswipe.goTo(nav_target);
    });
    $document.on("click", "." + CLASSES.CLOSE_BUTTON, function() {
      return photoswipe.close();
    });
    photoswipe.listen("afterChange", function() {
      var $current_image, current_index;
    ...
    photoswipe.listen("unbindEvents", function() {
      return $document.off("click", "." + CLASSES.NAV_LINK);
    });
    return photoswipe.listen("destroy", function() {
      var $card_image, $current_image, $nav;
      ...
      return $close_button.prepareTransition().removeClass(STATES.CLOSE_BUTTON.IS_ACTIVE);
    });
  });
});


},{"./../vendor/photoswipe.js":14,"./../vendor/photoswipe_ui.js":15}],4:[function(require,module,exports){
var ATTRS, BASE_CLASS, CLASSES, ImageGrid, VARIANTS, visible_count;
...
VARIANTS = {
  ITEM: {
    PLACEHOLDER: CLASSES.ITEM + "--placeholder"
  }
};
...
ImageGrid = function(node) {
  var $image_nodes, hidden_count, image_count, images, placeholder, update_placeholder;
  ...
  update_placeholder = function() {
    ...
    return placeholder.style.display = difference > 0 ? "" : "none";
  };
  update_placeholder();
  return $(window).on("resize", update_placeholder);
};

$(function() {
  var i, image_grid, len, ref, results;
  ref = document.querySelectorAll("." + CLASSES.BASE);
  results = [];
  for (i = 0, len = ref.length; i < len; i++) {
    image_grid = ref[i];
    results.push(ImageGrid(image_grid));
  }
  return results;
});
...
$(function() {
  var $document, $html, $page, modal_off;
  $html = $("html");
  $document = $(document);
  $page = $("." + CLASSES.PAGE);
  modal_off = function() {
    return $document.off("keydown").off("click", "." + VARIANTS.MODAL.MODAL_DARK).off("click", "." + CLASSES.ACTIVATOR + ", ." + CLASSES.DEACTIVATOR);
  };
  return $("." + CLASSES.ACTIVATOR + ", ." + CLASSES.DEACTIVATOR).on("click", function() {
    var $modal, modal_target;
    ...
    return $document.on('keydown', function(evt) {
      if ($modal.hasClass(STATES.MODAL.ACTIVE)) {
    ...
    }).on("click", "." + VARIANTS.MODAL.MODAL_DARK, function() {
      ...
      return modal_off();
    }).on("click", "." + CLASSES.MODAL_CONTENT, function(evt) {
...
$(function() {
  var active;
  active = false;
  $("." + CLASSES.ACTIVATOR).on("click", function() {
    var $activator;
    active = true;
    $activator = $(this);
    $activator.focus();
    return $activator.next("." + CLASSES.POPOVER).prepareTransition().addClass(STATES.ACTIVE);
  });
  $("." + CLASSES.BUTTON).on("click", function() {
    return $(this).closest("." + CLASSES.POPOVER).prepareTransition().removeClass(STATES.ACTIVE);
  });
  return $(document).on("mousedown", function(event) {
    var $target;
    $target = $(event.target);
    if ($target.closest("." + CLASSES.POPOVER)[0] || $target.closest("." + CLASSES.ACTIVATOR)[0]) {
      ...
      if (active) {
        active = false;
        return $("." + CLASSES.POPOVER).prepareTransition().removeClass(STATES.ACTIVE);
      }
    }
  });
});
...
var BASE_CLASS, CLASSES, LINK_TEXT, STATES;
...
STATES = {
  TEXT_CONTAINER: {
    OPEN: BASE_CLASS + "--is-open"
  }
};

$(function() {
  return $("." + CLASSES.TEXT_REVEALER).on("click", function() {
...
},{"./product.coffee":9}],9:[function(require,module,exports){
var CLASSES, STATES;
...
$(function() {
  return $("." + CLASSES.DETAILS).on("scroll", function() {
    if ($("." + CLASSES.DETAILS)[0].scrollHeight - $("." + CLASSES.DETAILS).scrollTop() <= $("." + CLASSES.DETAILS).outerHeight()) {
      return $("." + CLASSES.CONTENT).removeClass(STATES.FADED);
    } else {
      return $("." + CLASSES.CONTENT).addClass(STATES.FADED);
    }
  });
});
...
},{"./components/components.coffee":2,"./pages/pages.coffee":8,"./vendor/vendor.coffee":18}],11:[function(require,module,exports){
var Breakpoints;

Breakpoints = {
  card_breakaway: window.matchMedia("(min-width: 400px)"),
  card_flip: window.matchMedia("(min-width: 720px)")
};
...
},{"./../vendor/jquery-ui.min.js":13,"./../vendor/preparetransition.js":16,"./../vendor/social-buttons.js":17}]},{},[10]);
